...
public class SensorsRestController {
	...
	@ResponseBody
	public ResponseEntity<DeviceDTO> getPersonDetails(@PathVariable("code") final String code) {
		LOGGER.debug("REST call for getting a person by value: " + code);
		DeviceDTO deviceDTO = dynamapsViewService.getDeviceDetails(code);

		return new ResponseEntity<DeviceDTO>(deviceDTO, OK);
	}
